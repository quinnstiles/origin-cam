...
export async function finalizeSession(identifier, reason = "manual", isUserId = false) {
    ...
    // ====================================
    // 🌟 FIX: Pull fresh data from 'profiles' to ensure we calculate accurately
    ...
    const activeDbSeconds = currentProfile ? currentProfile.seconds : session.dbSeconds;
    ...
    // ====================================
    // 🌟 FIX: Updated table matching 'profiles' and column field tracking 'seconds'
    const { data, error } = await supabase
        .from("profiles")
        .update({ seconds: remainingSeconds })
        .eq("id", session.userId)
        .select();
