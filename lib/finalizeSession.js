...
export async function finalizeSession(identifier, reason = "manual", isUserId = false) {
    ...
    // ========================================================================
    // 🌟 AUTHORITATIVE SERVER-SIDE STREAM CUTOFF (ZERO-TRUST COMPLIANT)
    // ========================================================================
    if (reason === "balance-depleted" || reason === "stream-activity-lost" || reason === "manual") {

        if (session.decartSessionId) {
            console.log(`💥 [SERVER FORCE KILL] Evicting Decart stream ID: ${session.decartSessionId}`);

            try {
                // Target Decart's actual infrastructure identifier handle
                const decartRes = await fetch(`https://api.decart.ai/v1/realtime/sessions/${session.decartSessionId}`, {
                    method: "DELETE",
                    headers: {
                ...
                if (decartRes.ok) {
                    console.log(`✅ [UPSTREAM] Decart streaming pipeline severed directly by server authority.`);
                } else {
                    const errTxt = await decartRes.text();
                    console.log(`⚠️ [UPSTREAM] Decart execution warning: ${errTxt}`);
                }
            } catch (upstreamErr) {
                // Failsafe wrapper prevents external network down-times from breaking local DB balance calculation commits
                console.error("❌ Failed terminating upstream Decart link directly:", upstreamErr.message);
            }
        } else {
            console.log("⚠️ [FINALIZE] No upstream decartSessionId found mapping to this session profile.");
        }
    }
