...
export async function finalizeSession(identifier, reason = "manual", isUserId = false) {
    ...
    const sessionCreatedAt = session.createdAt; // Absolute server start clock
    ...
    try {
        ...
        // Calculate the exact elapsed lifespan of the streaming pipe on our server
        const now = Date.now();
        const localElapsedMs = now - sessionCreatedAt;

        // Convert milliseconds to seconds and round up cleanly
        const elapsedSeconds = Math.ceil(localElapsedMs / 1000);
