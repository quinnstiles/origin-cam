...
// Change getUserSession to simply return the session if it exists. 
// Let the routes decide if it's stale and handle the async finalization!
export function getUserSession(userId) {
    for (const session of sessions.values()) {
        if (session.userId === userId) {
            return session; // Hand over the session object to let caller handle state
        }
    }
    return null;
}
